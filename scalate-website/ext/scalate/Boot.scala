    ...
    if (_initialised.compareAndSet(false, true)) {
      ...
      def filter(m: Matcher): String = {
        ...
        engine.filter(filter_name) match {
          ...
          case None =>
            "<div class=\"macro error\"><p>filter not found: %s</p><pre>%s</pre></div>".format(filter_name, body)
        }
      }
      ...
      Markdown.macros :::= List(
        ...
        MacroDefinition(
          """\{pygmentize\_and\_compare::(.*?)\}(.*?)\{pygmentize\_and\_compare\}""",
          "s",
          pygmentize,
          true
        ),
