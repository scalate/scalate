...
abstract class ServerPageServlet extends HttpServlet
{
  ...
  override def service( request: HttpServletRequest, response: HttpServletResponse ): Unit = {
    ...
    val servlet = compiledPages.synchronized {
      ...
      val loadMethod = cachedPage match {
        case None =>
            if( bytecodeDirectory.exists  &&   findNewestTimestamp( bytecodeDirectory ) >= findURITimestamp( uri ) )
              'LoadPrebuilt
            else
              'Build
        case Some(page) =>
  ...
  private def findNewestTimestamp( path: File ): Long = {
    val children = path.listFiles match {
    ...
    if( children.length == 0 )
      ...
      children.foldLeft( path.lastModified )( (newestSoFar,file) => {
