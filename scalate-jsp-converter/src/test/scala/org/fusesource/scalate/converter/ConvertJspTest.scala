...
class ConvertJspTest extends AnyFunSuite with Log {
  ...
  assertConvert("""<c:url value='/foo'/>""", """${uri("/foo")}""")

  assertConvert("""blah <c:url value='/foo'/> blah""", """blah ${uri("/foo")} blah""")

  assertConvert("""blah <c:url value='/foo/${x}/bar/${y}'/> blah""", """blah ${uri("/foo/" + x + "/bar/" + y)} blah""")

  assertConvert("""<a href="<c:url value='/foo'/>">body</a>""", """<a href="${uri("/foo")}">body</a>""")

  assertConvert("""something <c:out value="${foo}"/> or other""", """something ${foo} or other""")
  ...
  assertConvert("""foo <c:set var="x" value='${foo}'/> whatnot""", """foo #{ var x = foo }# whatnot""")
  ...
  assertConvert("""foo <c:if test='${foo}'> bar </c:if> whatnot""", """foo #if(foo) bar #end whatnot""")
