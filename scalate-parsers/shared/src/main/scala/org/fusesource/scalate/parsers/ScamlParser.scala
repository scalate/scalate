...
sealed trait ScamlTextExpression extends Statement

case class Newline(skip: Boolean = true) extends Statement
case class EvaluatedText(code: Text, body: List[Statement], preserve: Boolean, sanitize: Option[Boolean], ugly: Boolean) extends ScamlTextExpression
case class LiteralText(text: List[Text], sanitize: Option[Boolean]) extends ScamlTextExpression {
...
import org.fusesource.scalate.parsers.ScamlParser._
