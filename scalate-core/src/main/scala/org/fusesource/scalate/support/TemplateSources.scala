...
class UriTemplateSource(uri: String, resourceLoader: ResourceLoader)
    extends UriResource(uri, resourceLoader)
    with TemplateSource
...
class CustomExtensionTemplateSource(source: TemplateSource, extensionName: String)
    extends DelegateResource
    with TemplateSource {
