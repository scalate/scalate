...
class Precompiler {
  ...
  def execute() = {
    ...
    val sourceDirs = ListBuffer[File]()
    if( warSourceDirectory!=null && warSourceDirectory.exists ) {
      sourceDirs += warSourceDirectory
    }
    if( resourcesSourceDirectory!=null && resourcesSourceDirectory.exists ) {
      sourceDirs += resourcesSourceDirectory
    }
    
    if(sourceDirs.isEmpty == null) {
      throw new IllegalArgumentException("The warSourceDirectory and/or resourcesSourceDirectory must be set and exist")
    }
    
    engine.resourceLoader = new FileResourceLoader(sourceDirs);
