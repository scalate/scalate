...
class IndentedParser extends RegexParsers() {
          ...
          if (elems.length == max) Success(elems.toList, in0)
            ...
            p(in0) match {
              case Success(x, rest) =>
                elems += x; applyp(rest)
              case ns: NoSuccess =>
                if (elems.length < min) { ns }
                else { Success(elems.toList, in0) }
            }
        }
        applyp(in)
      }
  ...
  def current_indent(strict: Boolean = false): Parser[Any] = {
    if (indent_level == 0) {
      ...
      var rc: Parser[Any] = mismatch_indent ~ err(
        "Inconsistent indent detected: indented with " + mismatch_indent_desc + " but previous lines were indented with " + indent_desc
      )
      if (strict) {
        // Look for indents that are too deep.
        rc |= repN(indent_level, indent_unit) ~ """[ \t]+""".r ~ err(
      ...
      if (indent_level > 0) {
        ...
        rc |= repN(indent_level - 1, indent_unit) ~ """[ \t]+""".r ~ err(
...
case class EvaluatedText(code: Text, body: List[Statement], preserve: Boolean, sanitize: Option[Boolean], ugly: Boolean)
    extends TextExpression
...
case class Element(
  tag: Option[Text],
  attributes: List[(Any, Any)],
  text: Option[TextExpression],
  body: List[Statement],
  trim: Option[Trim.Value],
  close: Boolean
) extends Statement
...
case class Attribute(kind: Text, name: Text, className: Text, defaultValue: Option[Text], autoImport: Boolean)
    extends Statement
...
class ScamlParser(val upto_type: String = UPTO_TYPE_SINGLE_LINE) extends IndentedParser() with ScalaParseSupport {
  ...
  def expression: Parser[Any] =
    (string_literal |
      ...
      (xml_ident | "{" ~> skip_whitespace(upto("}"), false) <~ "}") ^^ { x =>
        EvaluatedText(x, List(), true, Some(true), false)
      }
  ...
  def html_attribute_entry: Parser[(Any, Any)] =
    xml_ident ~ ("=" ~> string_literal) ^^ { case key ~ value =>
      (key, parse(skip_whitespace(literal_text(Some(true)), false), value.value))
    } |
      (xml_ident ~ ("=" ~> xml_ident) |
        xml_ident ~ ("=" ~ "{" ~> skip_whitespace(upto("}"), false) <~ "}")) ^^ { case key ~ value =>
  ...
  def haml_comment_statement = prefixed("-#", opt(dot) <~ nl) ~ rep(indent(opt_dot <~ nl)) ^^ { case text ~ body =>
    ScamlComment(text, body)
  }
  def html_comment_statement =
    prefixed("/", opt(prefixed("[", upto("]") <~ "]")) ~ opt(dot) <~ nl) ~ statement_block ^^ {
      case conditional ~ text ~ body => HtmlComment(conditional, text, body)
    }
  ...
  def text_statement = (prefixed("""\""", literal_text(None)) |
  ...
  def evaluated_statement = (opt("&" | "!") ~ ("=" | "~~" | "~")) ~! ((upto(nl) <~ nl) ~ statement_block) ^^ {
    case (sanitize ~ preserve) ~ (code ~ body) =>
      EvaluatedText(
        code,
        body,
        "=" != preserve,
  ...
  val attribute = skip_whitespace(
    opt(text("import")) ~ text("var" | "val") ~ scala_ident ~ (":" ~> qualified_type)
  ) ~ (opt_space ~> opt("=" ~ opt_space ~> upto(nl))) ^^ { case (p_import ~ p_kind ~ p_name ~ p_type) ~ p_default =>
    Attribute(p_kind, p_name, p_type, p_default, p_import.isDefined)
  }

  def attribute_statement = prefixed("-@", attribute <~ nl)

  def executed_statement =
    prefixed("-" ~ opt_space ~ nl, rep1(indent(opt_dot <~ nl))) ^^ { case code =>
      Executed(code, List())
    } |
      prefixed("-" ~ opt_space, dot <~ nl) ~ statement_block ^^ { case code ~ body =>
        Executed(code :: Nil, body)
      }

  def filter_statement = prefixed(
    ":",
    (rep(text("~" | "!" | "&")) ~ rep1sep(text("""[^: \t\r\n]+""".r), """[ \t]*:[ \t]*""".r)) <~ nl
  ) ~ rep(indent(opt_dot <~ nl)) ^^ { case (flags ~ code) ~ body =>
