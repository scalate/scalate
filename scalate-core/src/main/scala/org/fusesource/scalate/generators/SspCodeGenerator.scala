 ...
 */
package org.fusesource.scalate.generators

import org.fusesource.scalate.{ InvalidSyntaxException => _, _ }
import org.fusesource.scalate.parsers.{ ssp, _ }
import org.fusesource.scalate.parsers.ssp.{ AttributeFragment, CaseFragment, DoFragment, ElseFragment, ElseIfFragment, EndFragment, ExpressionFragment, ForFragment, IfFragment, ImportFragment, MatchFragment, OtherwiseFragment, ScriptletFragment, SetFragment, SspParser }
import org.fusesource.scalate.support.{ AbstractCodeGenerator, Code }
...
class SspCodeGenerator extends AbstractCodeGenerator[PageFragment] {
  ...
  private class SourceBuilder extends AbstractSourceBuilder[PageFragment] {
    ...
    def generate(fragment: PageFragment): Unit = {
      fragment match {
        case ssp.CommentFragment(code) => {
        }
        case ScriptletFragment(code) => {
          this << code :: Nil
        }
        case ssp.TextFragment(text) => {
          this << fragment.pos
          this << "$_scalate_$_context << ( " + asString(text) + " );"
        }
        case af: AttributeFragment => {
        }
        case ssp.DollarExpressionFragment(code) => {
  ...
  protected def transformSyntax(fragments: List[PageFragment]): List[PageFragment] = {
    ...
    val pass1 = fragments.filter {
      _ match {
        case t: ssp.TextFragment if (isMatch) =>
        ...
        case t: ssp.TextFragment =>
          ...
          if (str != transformed) {
            if (transformed.isEmpty) {
              ...
              Some(ssp.TextFragment(Text(transformed).setPos(t.pos)))
            }
          } else {
            Some(t)
          }
