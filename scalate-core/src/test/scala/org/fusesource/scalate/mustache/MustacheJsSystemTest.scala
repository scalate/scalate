...
class MustacheJsSystemTest extends MustacheTestSupport {
  ...
  mustacheTest(
    ...
    Map(
      "header" -> (() => "Colors"),
      "item" -> List(Item("red", true, "#Red"), Item("green", false, "#Green"), Item("blue", false, "#Blue")),
      "list" -> ((s: Scope) => s("item").get.asInstanceOf[List[?]].size > 0),
      "empty" -> ((s: Scope) => s("item").get.asInstanceOf[List[?]].size == 0)
    )
  )
  ...
  mustacheTest(
    ...
    Map(
      "it" -> Complex(
        "Colors",
        List(Item("red", true, "#Red"), Item("green", false, "#Green"), Item("blue", false, "#Blue"))
      )
    )
  )
  ...
  mustacheTest(
    ...
    Map(
      "top_nodes" -> Map(
        "contents" -> "1",
        "children" -> List(
          Map("contents" -> "2", "children" -> List(Map("contents" -> "3", "children" -> List()))),
          Map(
            "contents" -> "4",
            "children" -> List(Map("contents" -> "5", "children" -> List(Map("contents" -> "6", "children" -> List()))))
          )
        )
      )
    )
  ...
  mustacheTest("escaped", Map("title" -> (() => "Bear > Shark"), "entities" -> "&quot;"))
  ...
  if (runFailingTests) {
    ...
    mustacheTest(
      "higher_order_sections",
      "case class with string functor",
      Map("it" -> HigherOrder("Tater", "To tinker?"))
    )
  }
  ...
  mustacheTest(
    "null_string",
    Map("name" -> "Elise", "glytch" -> true, "binary" -> false, "value" -> null, "numeric" -> (() => Double.NaN))
  )
  ...
  mustacheTest(
    ...
    Map(
      "name" -> "name",
      "description" -> "desc",
      "terms" -> List(Map("name" -> "t1", "index" -> 0), Map("name" -> "t2", "index" -> 1))
    )
  )

  mustacheTest(
    "reuse_of_enumerables",
    Map("terms" -> List(Map("name" -> "t1", "index" -> 0), Map("name" -> "t2", "index" -> 1)))
  )
  ...
  mustacheTest(
    ...
    Map(
      "a_object" -> Map(
        ...
        "a_list" -> List(Map("label" -> "listitem1"), Map("label" -> "listitem2"))
      )
    )
  )
